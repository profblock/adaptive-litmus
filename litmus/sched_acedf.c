...
/*
 * In AC-EDF there is a acedf domain _per_ cluster
...
static noinline void unlink(struct task_struct* t)
{
	...
	if (t->rt_param.linked_on != NO_CPU) {
		 ...
		 * case.
		 *
		 * in AC-EDF case is should be somewhere in the queue for
...
static void check_for_preemptions(acedf_domain_t *cluster)
{
	 ...
	 * idle. */
	local = this_cpu_ptr(&acedf_cpu_entries);
...
{
	cpu_entry_t* entry = this_cpu_ptr(&acedf_cpu_entries);
	acedf_domain_t *cluster = entry->cluster;
...
static long acedf_activate_plugin(void)
{
	...
	printk(KERN_INFO "AC-EDF: Activate Plugin, cluster configuration = %d\n",
			cluster_config);
	...
	if (cluster_config == GLOBAL_CLUSTER) {
		...
		if (chk) {
			...
			printk(KERN_INFO "AC-EDF: Cluster configuration = %d "
	...
	if ((num_online_cpus() % cluster_size) != 0) {
		...
		printk(KERN_ERR "AC-EDF: Trying to group %d cpus in %d!\n",
				num_online_cpus(), cluster_size);
		return -1;
	}

	num_clusters = num_online_cpus() / cluster_size;
	printk(KERN_INFO "AC-EDF: %d cluster(s) of size = %d\n",
...
static int __init init_acedf(void)
{
	...
	if (!err) {
		...
		if (!fs)
			...
			printk(KERN_ERR "Could not allocate AC-EDF procfs dir.\n");
	}
	return err;
}
